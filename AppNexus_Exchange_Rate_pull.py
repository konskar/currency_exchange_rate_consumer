...
logging.basicConfig(filename='logging.log', level=logging.DEBUG, filemode='a', format='%(asctime)s - [%(levelname)s] :: %(message)s' )
...
def populate_exchange_rate_list_for_custom_date_range(): #  Generate ExchangeRateRowModel list for user input 
    ...
    if(len(appnexus_exchange_rate_list)==len(date_list) * len(currencies)):
    ...
    'Response objects: {}, Expected objects: {}'.format(len(payload_list), len(date_list) * len(currencies)))
